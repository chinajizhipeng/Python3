#coding=utf-8
import module 			#导入XX模块
module.fun() 			#这样调用~防止函数混淆
from module import fun 	#从model模块导入fun函数
fun() 					#这样调用
...
time.strptime('October 21,2015','%B %d,%Y')		#将字符串转换为datetime对象
...
#任何平台的任何编码 都能和Unicode互相转换  UTF-8与GBK互相转换，那就先把UTF-8转换成Unicode，再从Unicode转换成GBK，
...
##元组
#元组的不变性只适用于元组本身顶层而并非其内容 嵌套里层可修改
(1,2,3)
a,b,c = (1,2,3) 		#元组可以拆包
...
next(a) 
##闭包
#在函数内部在定义一个函数，并且这个函数用到了外面函数的变量，那么将这个函数以及用的一些变量成为闭包
...
r_object = re.compile(r'jzp \w+') 			#被替代的字符串的正则表达式
